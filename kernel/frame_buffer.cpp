...
Error FrameBuffer::Copy(Vector2D<int> pos, const FrameBuffer& src) {
    ...
    uint8_t* dst_buf =
        ...
        bytes_per_pixel *
            (config_.pixel_per_scan_line * copy_start_dst_y + copy_end_dst_x);
    ...
    for (int dy = 0; dy < copy_end_dst_y - copy_start_dst_y; ++dy) {
        ...
        dst_buf += bytes_per_pixel * config_.pixel_per_scan_line;
        src_buf += bytes_per_pixel * src.config_.pixel_per_scan_line;
    }
