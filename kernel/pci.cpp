...
/// @return CONFIG_ADDRESSに書き込むアドレス
...
uint16_t ReadDeviceId(uint16_t bus, uint8_t device, uint8_t function) {
    WriteAddress(MakeAddress(bus, device, function, 0x00));
    return ReadData() >> 16;
}
...
WithError<uint64_t> ReadBar(Device& device, unsigned int bar_index) {
    ...
    // NOTE: 以下64bit addressの場合の処理
    // NOTE: それぞれのbarは32bit幅なので、64bit
    // addressは連続した2つのbarを使って表現される

    // NOTE: barは5つしかないので、5つ目で64bit addressは表現できない
    if (bar_index >= 5) {
        return {0, MAKE_ERROR(Error::kIndexOutOfRange)};
    }

    // NOTE: addrが指しているbarの次のbarの値を取り出す
