...
const PixcelColor kDesktopBGColor{45, 118, 237};
const PixcelColor kDesktopFGColor{255, 255, 255};
...
void SwitchEhci2Xhci(const pci::Device& xhc_dev) {
    ...
    uint32_t exci2xhci_ports = pci::ReadConfReg(xhc_dev, 0xd4);
    pci::WriteConfReg(xhc_dev, 0xd0, exci2xhci_ports);
...
extern "C" void KernelMainNewStack(
    ...
    for (int i = 0; i < pci::num_device; ++i) {
        ...
        Log(kDebug, "%d.%d.%d: vend %04x, class %08x, head %02x, head %02x\n",
    ...
    while (true) {
        ...
        switch (msg.type) {
            case Message::kInterruptXHCI:
                while (auto err = ProcessEvent(xhc)) {
